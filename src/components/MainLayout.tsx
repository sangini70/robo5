...
export function MainLayout({ children, maxWidth = 'max-w-[1280px] xl:max-w-[1440px]' }: MainLayoutProps) {
  return (
    <>
      <header className="border-b border-gray-200 sticky top-0 bg-white/90 backdrop-blur-sm z-50">
        <div className={`${maxWidth} mx-auto px-6 lg:px-6 h-16 flex items-center justify-between`}>
      ...
      {/* Global Top AdSense */}
      <div className={`${maxWidth} mx-auto px-6 lg:px-6 w-full`}>
        <AdSense slotId="global-top-ad" />
      </div>

      <main className={`flex-grow w-full ${maxWidth} mx-auto px-6 lg:px-6 py-8`}>
        {children}
      </main>
